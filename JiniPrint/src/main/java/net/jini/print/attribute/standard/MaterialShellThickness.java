 ...
 * @author peter
 */
public class MaterialShellThickness extends IntegerSyntax implements Attribute {

    private static int check(int nanometres) {
	if (nanometres < 0) {
	    throw new IllegalArgumentException("nanometres must be zero or greater");
	}
	return nanometres;
    }

    public MaterialShellThickness(int nanometres) {
	super(check(nanometres));
    }

    @Override
    public Class<? extends Attribute> getCategory() {
	return MaterialShellThickness.class;
    }

    @Override
    public String getName() {
	return "material-shell-thickness";
    }
