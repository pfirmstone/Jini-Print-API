...
public abstract class CollectionSyntax {
    ...
    /**
     * Returns a string version of this attribute. The string version is
     * the concatenation of the string versions of the individual non-null media
     * characteristics, each preceded by its category name; for example:      
     * <CODE>"media-color: white, media-weight-metric: 
	 * 90 g/m**2"</CODE>.
     */
    @Override
    ...
    private static void addToString(StringBuilder result,
	    Attribute value) {
	if (value != null) {
	    if (result.length() > 0) {
		result.append(", ");
     ...
     * 
     * @return the attribute fields in the order listed by the relevant standard.
     */
    protected abstract Attribute[] getAttributes();
    
     /**
     * Determine whether this collection attribute is supported by the given collection
     * attribute.
     *
     * @param c collection attribute to test.
     *
     * @return If both AttributeCollections are the same type and
     * every non-wildcarded (non-null) field of this Media attribute
     * equals the corresponding field of given attribute, true
     * is returned. Otherwise, false is returned.
     */
    public boolean isSupportedBy(CollectionSyntax c){
	if (!this.getClass().isInstance(c)) return false;
	...
	for (int i = 0, l = mine.length; i<l; i++){
	    if (mine[i] != null && !mine[i].equals(given[i])) return false;
	}
	return true;
    }
