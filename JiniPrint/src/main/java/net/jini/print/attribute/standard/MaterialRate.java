 ...
 * @author peter
 */
public class MaterialRate extends IntegerSyntax implements Attribute {
    
    private static final long serialVersionUID = 1L;
    
    private static int check(int rate){
	if ( 0 < rate ) return rate;
	throw new IllegalArgumentException("rate must be 1 or greater");
    }
    
    public MaterialRate(int rate){
	super(check(rate));
    }

    @Override
    public Class<? extends Attribute> getCategory() {
	return MaterialRate.class;
    }
